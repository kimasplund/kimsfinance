...
class TestRenderPNFChart:
    ...
    def test_basic_rendering(self):
        ...
        img = render_pnf_chart(ohlc, volume, width=1200, height=800, box_size=2.0, reversal_boxes=3)
    ...
    def test_render_no_antialiasing(self):
        ...
        img = render_pnf_chart(ohlc, volume, width=800, height=600, enable_antialiasing=False)
    ...
    def test_render_with_grid(self):
        ...
        img_with_grid = render_pnf_chart(ohlc, volume, width=800, height=600, show_grid=True)
        img_without_grid = render_pnf_chart(ohlc, volume, width=800, height=600, show_grid=False)
    ...
    def test_render_auto_box_size(self):
        ...
        img = render_pnf_chart(
            ohlc, volume, width=1200, height=800, box_size=None, reversal_boxes=3  # Auto-calculate
        )
    ...
    def test_render_different_reversal_boxes(self):
        ...
        for reversal_boxes in [2, 3, 5, 10]:
            img = render_pnf_chart(
                ohlc, volume, width=1200, height=800, box_size=2.0, reversal_boxes=reversal_boxes
            )
            assert isinstance(img, Image.Image)
...
class TestPNFIntegration:
    ...
    def test_full_workflow(self):
        ...
        # Step 2: Render chart
        img = render_pnf_chart(
            ohlc, volume, width=1920, height=1080, box_size=0.5, reversal_boxes=3, theme="modern"
        )
