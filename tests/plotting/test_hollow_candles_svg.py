...
def test_render_hollow_candles_svg_basic():
    ...
    from kimsfinance.plotting import render_hollow_candles_svg
    ...
    svg_string = render_hollow_candles_svg(
        ohlc_dict, volume_data, width=800, height=600, theme="classic"
    )
...
def test_render_hollow_candles_svg_no_volume():
    ...
    svg_string = render_hollow_candles_svg(ohlc_dict, volume=None, width=800, height=600)
...
def test_render_hollow_candles_svg_custom_colors():
    ...
    svg_string = render_hollow_candles_svg(
        ohlc_dict, volume=None, bg_color="#FFFFFF", up_color="#00FF00", down_color="#FF0000"
    )
...
def test_hollow_candles_svg_via_plot_api():
    ...
    try:
        # Render via plot() API
        result = plot(
            df, type="hollow_and_filled", volume=True, savefig=tmp_path, width=800, height=600
        )
...
def test_hollow_vs_filled_candles_svg():
    ...
    ohlc_dict = {
        ...
        "close": np.array(
            [102.0, 98.0]
        ),  # First bullish (close > open), second bearish (close < open)
    }

    svg_string = render_hollow_candles_svg(ohlc_dict, volume=None, theme="classic")
