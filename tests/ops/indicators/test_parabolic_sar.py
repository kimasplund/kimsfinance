...
class TestParabolicSAR:
    ...
    def test_custom_parameters(self, sample_data):
        ...
        result1 = calculate_parabolic_sar(
            highs, lows, af_start=0.01, af_increment=0.01, af_max=0.1, engine="cpu"
        )

        result2 = calculate_parabolic_sar(
            highs, lows, af_start=0.03, af_increment=0.03, af_max=0.3, engine="cpu"
        )
    ...
    def test_sar_within_reasonable_range(self, sample_data):
        ...
        assert np.all(
            valid_sar >= min_price - 0.2 * price_range
    ...
    def test_known_values_simple(self):
        ...
        result = calculate_parabolic_sar(
            highs, lows, af_start=0.02, af_increment=0.02, af_max=0.2, engine="cpu"
        )
    ...
    def test_invalid_engine(self, sample_data):
        """Test that invalid engine parameter raises ConfigurationError."""
        highs, lows = sample_data
