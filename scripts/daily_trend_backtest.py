...
def run_daily_trend_backtest():
    ...
    for i in range(len(df)):
        ...
        if position == 1:
            ...
            if row['Low'] <= row['10_Low']:
                ...
                if row['Open'] < exit_price: exit_price = row['Open'] # Gap down
            ...
            if row['High'] >= row['10_High']:
                ...
                if row['Open'] > exit_price: exit_price = row['Open'] # Gap up
        ...
        if position == 0:
            ...
            if row['High'] >= row['20_High']:
                ...
                if row['Open'] > entry_price: entry_price = row['Open']
    ...
    if position != 0:
        ...
        trades.append({
            ...
            'Entry': entry_price,
            'Exit': exit_price,
            'Return_%': ((exit_price - entry_price) / entry_price) * 100 if position==1 else ((entry_price - exit_price) / entry_price) * 100,
            'Net_PnL': net_pnl,
            'Capital': capital
        })
