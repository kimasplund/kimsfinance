    ...
    for current_time in all_timestamps:
        ...
        for ticker, df in dfs.items():
            ...
            if strategy == "EMA_Crossover":
                ...
                if is_trending_up and prev_prev_row['EMA_9'] <= prev_prev_row['EMA_20'] and prev_row['EMA_9'] > prev_row['EMA_20']:
...
def main():
    ...
    with open(report_path, "w") as f:
        ...
        for _, row in df_res.iterrows():
            ...
            f.write(f"| {row['Timeframe']} | {row['Strategy']} | {targets} | {row['Total_Trades']} | {row['Win_Rate']:.1f}% | ${row['Net_Profit']:,.2f} |\n")
