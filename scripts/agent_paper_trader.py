...
def run_simulation():
            ...
            if pos is not None:
                ...
                if low <= pos["stop_loss"]:
                    ...
                    if pos["size"] == 1.0:
                        ...
                        r_realized = 0.0
                        result = (
                            "Win"  # Still a winning trade overall since we secured 1.5R on half
                        )
                    ...
                    trades.append(
                        {
                            ...
                            "Skill": pos["skill"],
                            "Result": result,
                            "Total_R": pos["secured_r"] + r_realized,
    ...
    with open(report_path, "w") as f:
        ...
        f.write(
            "This simulation implemented the contextual Trend Filter and the advanced risk management rules from **Book 11: The Risk Architect** (Scaling Out at 1.5R, Breakeven Trailing, and -3R Daily Limits).\n\n"
        )
        if USE_BINANCE:
            f.write(
                "**Parameters**: 1 Year of 5-Minute Data (Binance Vision), $10,000 Starting Account, 1% Risk ($100) per trade.\n\n"
            )
        else:
            f.write(
                "**Parameters**: 1 Year of 1-Hour Data (yfinance), $10,000 Starting Account, 1% Risk ($100) per trade.\n\n"
            )
        ...
        f.write(
            f"- **Final Account Balance**: ${current_capital:,.2f} (Net Profit: ${current_capital - INITIAL_CAPITAL:,.2f})\n"
        )
