...
def main():
    ...
    with open(report_path, "w") as f:
        ...
        f.write(
            "| Timeframe | Targets (T1 / T2) | Total Trades | Win Rate | EV per Trade | Avg Win | Avg Loss | Net Profit |\n"
        )
        f.write(
            "|-----------|-------------------|--------------|----------|--------------|---------|----------|------------|\n"
        )
        for _, row in df_res.iterrows():
            ...
            f.write(
                f"| {row['Timeframe']} | {targets_str} | {row['Total_Trades']} | {row['Win_Rate']:.1f}% | ${row['EV']:,.2f} | ${row['Avg_Win']:,.2f} | ${row['Avg_Loss']:,.2f} | ${row['Net_Profit']:,.2f} |\n"
            )
