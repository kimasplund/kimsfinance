...
def main():
    ...
    if not info.get("gpu_available"):
        print(
            "GPU not available. Falling back to CPU, but kimsfinance Rust core will still execute extremely fast."
        )
    else:
    ...
    config = BacktestConfig(
        initial_capital=10000.0, trading_fee=0.00045, slippage=0.0001  # Taker fee
    )
