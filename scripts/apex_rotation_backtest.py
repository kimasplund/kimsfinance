...
def run_apex_rotation():
    ...
    for i, current_date in enumerate(dates):
        ...
        portfolio_history.append(
            {"Date": current_date, "Value": current_value, "Holding": current_holding}
        )
        ...
        if target_asset != current_holding:
            ...
            if current_holding != "CASH":
                ...
                trade_log.append(
                    f"[{current_date.date()}] SOLD {current_holding} at ${exit_price:.2f} | Value: ${capital:.2f}"
                )
            ...
            if target_asset != "CASH":
                ...
                trade_log.append(
                    f"[{current_date.date()}] BOUGHT {target_asset} at ${entry_price:.2f}"
                )
    ...
    print("=" * 50)
    print(
        f"Total Rotations Executed: {len(trade_log) // 2}"
    )  # Divide by 2 because buy/sell are 2 logs
