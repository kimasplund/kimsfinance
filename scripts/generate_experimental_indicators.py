...
def load_data_with_indicators(csv_path: str, n_candles: int = 100,
                               warmup_candles: int = 50, start_row: int = 600) -> pl.DataFrame:
...
def create_adx_panel_chart(df_pandas, output_path: Path, title: str) -> float:
    ...
    try:
        ...
        apds = [
            mpf.make_addplot(adx_data['adx'], panel=2, color='purple',
                            ylabel='ADX', secondary_y=False, width=1.5),
...
def main():
    ...
    base_output_dir = Path(__file__).parent.parent / "docs" / "sample_charts" / "indicators" / "experimental"
    ...
    charts = [
        ("01_adx_directional_indicators.webp",
         lambda: create_adx_panel_chart(df_pandas, base_output_dir / "01_adx_directional_indicators.webp",
                                        "ADX with Directional Indicators (+DI, -DI)")),

        ("02_regime_classification.webp",
         lambda: create_regime_overlay_chart(df_pandas, base_output_dir / "02_regime_classification.webp",
                                             "Market Regime Classification (Experimental)")),

        ("03_price_with_ma.webp",
         lambda: create_simple_chart(df_pandas, base_output_dir / "03_price_with_ma.webp",
                                     "Price with Moving Averages", mav=(20, 50))),

        ("04_clean_price_action.webp",
         lambda: create_simple_chart(df_pandas, base_output_dir / "04_clean_price_action.webp",
                                     "Clean Price Action (No Indicators)")),

        ("05_fibonacci_periods.webp",
         lambda: create_simple_chart(df_pandas, base_output_dir / "05_fibonacci_periods.webp",
                                     "Fibonacci Periods (8, 21, 55)", mav=(8, 21, 55))),
    ]
