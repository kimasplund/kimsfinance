...
class BacktestReport:
    ...
    def _create_cover_page(self) -> list:
        ...
        table.setStyle(
            TableStyle(
                [
                    ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#%02x%02x%02x" % self.config.primary_color)),
    ...
    def _create_equity_curve_page(self) -> list:
        ...
        if self.config.include_rolling_sharpe:
            ...
            returns = self.equity_curve.pct_change().dropna()
            sharpe_img = create_rolling_sharpe(returns, width=700, height=250, dpi=self.config.chart_dpi)
    ...
    def _create_performance_analysis(self) -> list:
        ...
        returns = self.equity_curve.pct_change().dropna()
        dist_img = create_returns_distribution(returns, width=600, height=350, dpi=self.config.chart_dpi)
    ...
    def _create_appendix(self) -> list:
        ...
        for i, (idx, trade) in enumerate(trades_to_show.iterrows(), 1):
            row = [
                str(i),
                trade.get("entry_time", "N/A").strftime("%Y-%m-%d %H:%M") if hasattr(trade.get("entry_time", ""), "strftime") else str(trade.get("entry_time", "N/A")),
                trade.get("exit_time", "N/A").strftime("%Y-%m-%d %H:%M") if hasattr(trade.get("exit_time", ""), "strftime") else str(trade.get("exit_time", "N/A")),
