...
def _get_operation_func(operation: str):
    ...
    from kimsfinance.ops.indicators import calculate_atr, calculate_rsi, calculate_stochastic_oscillator
    ...
    if operation == "atr":
        ...
        return lambda df, engine: calculate_stochastic_oscillator(df['high'], df['low'], df['close'], engine=engine)
    else:
