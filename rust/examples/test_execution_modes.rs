...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Processed {} strategies in {:.2}ms", params_traditional.len(), time_trad);
    ...
    for rsi_period in 10..15 {
        for buy_thresh in 20..30 {
            for sell_thresh in 70..80 {
                params_fused.push(vec![rsi_period as f64, buy_thresh as f64, sell_thresh as f64]);
            }
        }
    }
    ...
    println!("✅ Processed {} strategies in {:.2}ms", params_fused.len(), time_fused);
    ...
    for rsi_period in 12..15 {
        for buy_thresh in 25..35 {
            for sell_thresh in 70..73 {
                params_auto.push(vec![rsi_period as f64, buy_thresh as f64, sell_thresh as f64]);
            }
        }
    }
    ...
    println!("✅ Processed {} strategies in {:.2}ms", params_auto.len(), time_auto);
    ...
    println!("Traditional: {:.2}ms for {} strategies", time_trad, params_traditional.len());
    println!("Fused:       {:.2}ms for {} strategies", time_fused, params_fused.len());
    println!("Auto:        {:.2}ms for {} strategies", time_auto, params_auto.len());
    println!();
    ...
    if speedup >= 1.31 {
        println!("\n✅ SUCCESS: Fused kernel achieves {:.2}x speedup!", speedup);
        println!("   Target: ≥1.31x, Typical: 2-4x");
    } else {
        println!("\n⚠️  WARNING: Speedup ({:.2}x) below 1.31x target", speedup);
    }
