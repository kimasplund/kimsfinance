...
// ── OHLCV container ────────────────────────────────────────────────────────────
struct OHLCVData {
    timestamps: Vec<i64>, open: Vec<f64>, high: Vec<f64>,
    low: Vec<f64>, close: Vec<f64>, volume: Vec<f64>,
}
impl OHLCVData {
    fn len(&self) -> usize { self.timestamps.len() }
    fn slice(&self, s: usize, e: usize) -> OHLCVData {
        OHLCVData {
            timestamps: self.timestamps[s..e].to_vec(), open: self.open[s..e].to_vec(),
            high: self.high[s..e].to_vec(), low: self.low[s..e].to_vec(),
            close: self.close[s..e].to_vec(), volume: self.volume[s..e].to_vec(),
        }
    }
    fn arrs(&self) -> (Array1<f64>, Array1<f64>, Array1<f64>, Array1<f64>, Array1<f64>) {
        (Array1::from_vec(self.open.clone()), Array1::from_vec(self.high.clone()),
         Array1::from_vec(self.low.clone()),  Array1::from_vec(self.close.clone()),
...
fn load_ohlcv(path: &str) -> Result<OHLCVData, Box<dyn std::error::Error>> {
    ...
    let mut d = OHLCVData { timestamps:vec![], open:vec![], high:vec![], low:vec![], close:vec![], volume:vec![] };
    for batch in reader {
        let batch = batch?;
        ...
        let ns: Vec<i64> = if let Some(a) = tc.as_any().downcast_ref::<TimestampNanosecondArray>() {
            ...
            (0..batch.num_rows()).map(|i| a.value(i)).collect()
        } else { return Err("bad ts".into()); };
        macro_rules! col { ($n:expr) => {
            batch.column_by_name($n).and_then(|c| c.as_any().downcast_ref::<Float64Array>())
                 .ok_or(concat!("no ",$n))? } }
        let (oa,ha,la,ca,va)=(col!("Open"),col!("High"),col!("Low"),col!("Close"),col!("Volume"));
        for i in 0..batch.num_rows() {
            d.timestamps.push(ns[i]/1_000_000); d.open.push(oa.value(i)); d.high.push(ha.value(i));
            d.low.push(la.value(i)); d.close.push(ca.value(i)); d.volume.push(va.value(i));
        }
    }
    Ok(d)
}
#[cfg(not(feature = "data-downloaders"))]
fn load_ohlcv(_: &str) -> Result<OHLCVData, Box<dyn std::error::Error>> { Err("needs data-downloaders".into()) }
...
impl EMARegime {
    fn new(period: usize, buf: f64, cooldown: usize, pos_frac: f64) -> Self {
        Self {
            period, buf, cooldown, pos_frac,
            ema: 0.0, alpha: 2.0 / (period as f64 + 1.0),
            prev_pos: 0.0, cd: 0, bars: 0, cur_px: 0.0,
        }
    }
}
...
impl Strategy for EMARegime {
    fn on_data(&mut self, bar: &OHLCVBar, _ind: &IndicatorValues) -> Signal {
        ...
        let result = if nxt_pos != self.prev_pos {
            ...
            match (self.prev_pos as i32, nxt_pos as i32) {
                (0, 1) =>   { self.prev_pos = nxt_pos; Signal::Buy },    // Flat → Long
                (0, -1) =>  { self.prev_pos = nxt_pos; Signal::Short },  // Flat → Short
                (1, 0) =>   { self.prev_pos = nxt_pos; Signal::Sell },   // Long → Flat
                (1, -1) =>  { self.prev_pos = nxt_pos; Signal::Short },  // Long → Short (flip)
                (-1, 0) =>  { self.prev_pos = nxt_pos; Signal::Cover },  // Short → Flat
                (-1, 1) =>  { self.prev_pos = nxt_pos; Signal::Buy },    // Short → Long (flip)
    ...
    fn position_size(&self, _equity: f64, _: Signal) -> f64 {
        if self.cur_px > 0.0 { 10_000.0 * self.pos_frac / self.cur_px } else { 0.0 }
    }
...
// ── Helpers ───────────────────────────────────────────────────────────────────
#[derive(Debug, Clone)]
struct Res { ret: f64, sharpe: f64, dd: f64, wr: f64, trades: usize, equity: f64 }
...
fn bt(s: &mut EMARegime, d: &OHLCVData, cfg: &BacktestConfig) -> Res {
    ...
    match eng.run(s, &d.timestamps, &o, &h, &l, &c, &v) {
        Ok(r) => Res { ret: r.total_return, sharpe: r.sharpe_ratio, dd: r.max_drawdown,
                       wr: r.win_rate, trades: r.num_trades, equity: r.final_equity },
        Err(e) => { eprintln!("bt error: {:?}", e);
            Res { ret: -999.0, sharpe: -99.0, dd: 100.0, wr: 0.0, trades: 0, equity: 10_000.0 } }
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  BTC ${:.0}→${:.0}  ({:+.1}% buy-and-hold)", btc.close[0], btc.close[n-1], bnh);

    let is_n = (n as f64 * 0.75) as usize;
    let (btc_is, btc_oos) = (btc.slice(0, is_n), btc.slice(is_n, n));

    let cfg = BacktestConfig {
        initial_capital: 10_000.0, trading_fee: 0.0004, slippage: 0.00015,
        execution_latency_ms: 50, use_gpu: cfg!(feature = "gpu"), force_cpu: false,
    };
    ...
    println!("  IS Result: Return={:+.2}% Sharpe={:.3} DD={:.2}% WR={:.1}% Trades={}",
    ...
    for i in 0..3 {
        ...
        if oe > e {
            ...
            println!("  Q{}: IS Shr={:.3} {:+.2}% ({} t)  │  OOS Shr={:.3} {:+.2}% ({} t)  {}",
                     i+1, ri.sharpe, ri.ret, ri.trades, ro.sharpe, ro.ret, ro.trades,
                     if ro.ret > 0.0 { "✅" } else { "❌" });
        }
    }
    ...
    println!("\n  {:>22} {:>12} {:>12}", "Metric", "IS (train)", "OOS (test)");
    println!("  {}", "─".repeat(48));
    println!("  {:>22} {:>+12.2}% {:>+12.2}%", "Return", ri.ret, ro.ret);
    println!("  {:>22} {:>12.3}  {:>12.3}", "Sharpe", ri.sharpe, ro.sharpe);
    ...
    println!("  {:>22} {:>12.2}  {:>12.2}", "Final $", ri.equity, ro.equity);
    println!("\n  IS→OOS Sharpe degradation: {:.1}%", deg);

    // ── Phase 5: Multi-Symbol ─────────────────────────────────────────────
    println!("\nPHASE 5: Multi-Symbol (same params, no refitting)");
    println!("──────────────────────────────────────────────────");
    let syms = [("BTCUSDT OOS", &btc_oos), ("ETHUSDT full", &eth), ("SOLUSDT full", &sol)];
    println!("  {:>16} {:>10} {:>8} {:>10} {:>8}", "Symbol", "Return%", "Sharpe", "MaxDD%", "Trades");
    println!("  {}", "─".repeat(55));
    let mut pos_cnt = 0usize;
    let multi: Vec<_> = syms.iter()
        .map(|(nm, d)| {
    ...
    for (nm, r, ok) in &multi {
        println!("  {:>16} {:>+10.2} {:>8.3} {:>10.2} {:>8}  {}",
                 nm, r.ret, r.sharpe, r.dd, r.trades, if *ok { "✅" } else { "❌" });
    }
    ...
    println!("\n  Fixed sizing: {:.0}% of ${}k = ${:.0}/trade", core.3*100.0, (cap/1000.0) as i64, avg_pos);
    println!("  Fee/RT: ${:.2}  ×  {} trades  =  ${:.2} total", fee_per_rt, ro.trades, total_fees);
    println!();
    println!("  Gross (before fees):  {:>+8.2}%  (${:.2})", gross_pct, gross_usd);
    println!("  Fees:                 {:>+8.2}%  (-${:.2})", -(total_fees/cap*100.0), total_fees);
    println!("  Net (actual):         {:>+8.2}%  (${:.2})", ro.ret, net_usd);
    if drag.is_finite() { println!("  Fee drag:             {:>8.1}% of gross", drag); }
    println!("\n  Realistic (50% worse execution):");
    println!("    Fees ${:.2}  →  Return {:+.2}%  {}", real_fees, real_pct,
             if real_pct > 0.0 { "✅ Survives" } else { "❌ Fails" });
    ...
    let checks: &[(&str, bool)] = &[
        ...
        ("Multi-symbol >= 2/3 positive", pos_cnt as f64 / multi.len().max(1) as f64 >= 2.0 / 3.0),
    ...
    println!("\n  Summary:");
    println!("  {:>18} {:>10} {:>8} {:>10} {:>7}", "Period", "Return%", "Sharpe", "MaxDD%", "Trades");
    println!("  {}", "─".repeat(55));
    println!("  {:>18} {:>+10.2} {:>8.3} {:>10.2} {:>7}", "IS (train)", ri.ret, ri.sharpe, ri.dd, ri.trades);
    println!("  {:>18} {:>+10.2} {:>8.3} {:>10.2} {:>7}", "OOS (unseen)", ro.ret, ro.sharpe, ro.dd, ro.trades);
